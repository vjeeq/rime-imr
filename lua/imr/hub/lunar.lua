local convert = require("imr.hub._util").convert

local cTianGan = {
    "甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸",
}
local cDiZhi = {
    "子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥",
}
local cShuXiang = {
    "鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪",
}
local cDayName = {
    "初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
    "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十",
    "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十",
}
local cMonName = {
    "正月", "二月", "三月", "四月", "五月", "六月",
    "七月", "八月", "九月", "十月", "冬月", "腊月",
}

local wNongliData = {
    "AB500D2", "4BD0883", "4AE00DB", "A5700D0", "54D0581", "D2600D8", "D9500CC", "655147D",
    "56A00D5", "9AD00CA", "55D027A", "4AE00D2", "A5B0682", "A4D00DA", "D2500CE", "D25157E",
    "B5500D6", "56A00CC", "ADA027B", "95B00D3", "49717C9", "49B00DC", "A4B00D0", "B4B0580",
    "6A500D8", "6D400CD", "AB5147C", "2B600D5", "95700CA", "52F027B", "49700D2", "6560682",
    "D4A00D9", "EA500CE", "6A9157E", "5AD00D6", "2B600CC", "86E137C", "92E00D3", "C8D1783",
    "C9500DB", "D4A00D0", "D8A167F", "B5500D7", "56A00CD", "A5B147D", "25D00D5", "92D00CA",
    "D2B027A", "A9500D2", "B550781", "6CA00D9", "B5500CE", "535157F", "4DA00D6", "A5B00CB",
    "457037C", "52B00D4", "A9A0883", "E9500DA", "6AA00D0", "AEA0680", "AB500D7", "4B600CD",
    "AAE047D", "A5700D5", "52600CA", "F260379", "D9500D1", "5B50782", "56A00D9", "96D00CE",
    "4DD057F", "4AD00D7", "A4D00CB", "D4D047B", "D2500D3", "D550883", "B5400DA", "B6A00CF",
    "95A1680", "95B00D8", "49B00CD", "A97047D", "A4B00D5", "B270ACA", "6A500DC", "6D400D1",
    "AF40681", "AB600D9", "93700CE", "4AF057F", "49700D7", "64B00CC", "74A037B", "EA500D2",
    "6B50883", "5AC00DB", "AB600CF", "96D0580", "92E00D8", "C9600CD", "D95047C", "D4A00D4",
    "DA500C9", "755027A", "56A00D1", "ABB0781", "25D00DA", "92D00CF", "CAB057E", "A9500D6",
    "B4A00CB", "BAA047B", "AD500D2", "55D0983", "4BA00DB", "A5B00D0", "5171680", "52B00D8",
    "A9300CD", "795047D", "6AA00D4", "AD500C9", "5B5027A", "4B600D2", "96E0681", "A4E00D9",
    "D2600CE", "EA6057E", "D5300D5", "5AA00CB", "76A037B", "96D00D3", "4AB0B83", "4AD00DB",
    "A4D00D0", "D0B1680", "D2500D7", "D5200CC", "DD4057C", "B5A00D4", "56D00C9", "55B027A",
    "49B00D2", "A570782", "A4B00D9", "AA500CE", "B25157E", "6D200D6", "ADA00CA", "4B6137B",
    "93700D3", "49F08C9", "49700DB", "64B00D0", "68A1680", "EA500D7", "6AA00CC", "A6C147C",
    "AAE00D4", "92E00CA", "D2E0379", "C9600D1", "D550781", "D4A00D9", "DA400CD", "5D5057E",
    "56A00D6", "A6C00CB", "55D047B", "52D00D3", "A9B0883", "A9500DB", "B4A00CF", "B6A067F",
    "AD500D7", "55A00CD", "ABA047C", "A5A00D4", "52B00CA", "B27037A", "69300D1", "7330781",
    "6AA00D9", "AD500CE", "4B5157E", "4B600D6", "A5700CB", "54E047C", "D1600D2", "E960882",
    "D5200DA", "DAA00CF", "6AA167F", "56D00D7", "4AE00CD", "A9D047D", "A2D00D4", "D1500C9",
    "F250279", "D5200D1",
}

local function Dec2bin(n)
    local t, tables = tonumber(n), {}
    while math.floor(t / 2) >= 1 do
        local t1 = t % 2
        table.insert(tables, 1, t1 > 0 and 1 or 0)
        t = math.floor(t / 2)
        if t == 1 then table.insert(tables, 1, 1) end
    end
    return string.gsub(table.concat(tables), "^[0]+", "")
end

local function Atoi(x, inPuttype, outputtype)
    if tonumber(inPuttype) == 2 then
        return tonumber(tostring(x), 2)
    elseif tonumber(inPuttype) == 10 then
        if tonumber(outputtype) == 2 then return Dec2bin(tonumber(x))
        else return string.format("%x", x) end
    elseif tonumber(inPuttype) == 16 then
        if tonumber(outputtype) == 2 then return Dec2bin(tonumber(tostring(x), 16))
        else return tonumber(tostring(x), 16) end
    end
end

local function Analyze(Data)
    local rtn1 = Atoi(string.sub(Data, 1, 3), 16, 2)
    if string.len(rtn1) < 12 then rtn1 = "0" .. rtn1 end
    local rtn2 = string.sub(Data, 4, 4)
    local rtn3 = Atoi(string.sub(Data, 5, 5), 16, 10)
    local rtn4 = Atoi(string.sub(Data, -2, -1), 16, 10)
    if string.len(rtn4) == 3 then rtn4 = "0" .. Atoi(string.sub(Data, -2, -1), 16, 10) end
    return { rtn1, rtn2, rtn3, rtn4 }
end

local function IsLeap(y)
    y = tonumber(y)
    if not y then return nil end
    if (y % 400 ~= 0 and y % 4 == 0) or y % 400 == 0 then
        return 366
    end
    return 365
end

local function leaveDate(y)
    local total = 0
    local day = IsLeap(tonumber(string.sub(y, 1, 4))) > 365
        and { 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }
        or { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }
    if tonumber(string.sub(y, 5, 6)) > 1 then
        for i = 1, tonumber(string.sub(y, 5, 6)) - 1 do total = total + day[i] end
        total = total + tonumber(string.sub(y, 7, 8))
    else
        return tonumber(string.sub(y, 7, 8))
    end
    return tonumber(total)
end

local function diffDate(date1, date2)
    local total = 0
    date1, date2 = tostring(date1), tostring(date2)
    if tonumber(date2) > tonumber(date1) then
        local n = tonumber(string.sub(date2, 1, 4)) - tonumber(string.sub(date1, 1, 4))
        if n > 1 then
            for i = 1, n - 1 do total = total + IsLeap(tonumber(string.sub(date1, 1, 4)) + i) end
            total = total + leaveDate(tonumber(string.sub(date2, 1, 8)))
                + IsLeap(tonumber(string.sub(date1, 1, 4)))
                - leaveDate(tonumber(string.sub(date1, 1, 8)))
        elseif n == 1 then
            total = IsLeap(tonumber(string.sub(date1, 1, 4)))
                - leaveDate(tonumber(string.sub(date1, 1, 8)))
                + leaveDate(tonumber(string.sub(date2, 1, 8)))
        else
            total = leaveDate(tonumber(string.sub(date2, 1, 8))) - leaveDate(tonumber(string.sub(date1, 1, 8)))
        end
    elseif tonumber(date2) == tonumber(date1) then
        return 0
    else
        return -1
    end
    return total
end

local function Date2LunarDate(Gregorian)
    Gregorian = tostring(Gregorian)
    local Year = tonumber(string.sub(Gregorian, 1, 4))
    local Month = tonumber(string.sub(Gregorian, 5, 6))
    local Day = tonumber(string.sub(Gregorian, 7, 8))
    if Year > 2100 or Year < 1899 or Month > 12 or Month < 1 or Day < 1 or Day > 31
        or string.len(Gregorian) < 8 then
        return "无效日期", "无效日期"
    end

    local Pos = Year - 1900 + 2
    local Data0 = wNongliData[Pos - 1]
    local Data1 = wNongliData[Pos]
    local tb1 = Analyze(Data1)
    local MonthInfo = tb1[1]
    local LeapInfo = tb1[2]
    local Leap = tb1[3]
    local Newyear = tb1[4]
    local Date1 = Year .. Newyear
    local Date3 = diffDate(Date1, Gregorian)

    if Date3 < 0 then
        tb1 = Analyze(Data0)
        Year = Year - 1
        MonthInfo = tb1[1]
        LeapInfo = tb1[2]
        Leap = tb1[3]
        Newyear = tb1[4]
        Date1 = Year .. Newyear
        Date3 = diffDate(Date1, Gregorian)
    end

    Date3 = Date3 + 1
    local LYear = Year
    local thisMonthInfo
    if Leap > 0 then
        thisMonthInfo = string.sub(MonthInfo, 1, tonumber(Leap)) .. LeapInfo
            .. string.sub(MonthInfo, Leap + 1)
    else
        thisMonthInfo = MonthInfo
    end

    local LMonth, LDay, Isleap
    for i = 1, 13 do
        local days = 29 + string.sub(thisMonthInfo, i, i)
        if Date3 > days then
            Date3 = Date3 - days
        else
            if Leap > 0 then
                if Leap >= i then LMonth = i; Isleap = 0
                else
                    LMonth = i - 1
                    Isleap = (i - Leap == 1) and 1 or 0
                end
            else
                LMonth = i; Isleap = 0
            end
            LDay = math.floor(Date3)
            break
        end
    end

    local LunarMonth = Isleap > 0 and "闰" .. cMonName[LMonth] or cMonName[LMonth]
    local _nis = tostring(LYear)
    local _LunarYears = ""
    for i = 1, string.len(_nis) do
        _LunarYears = _LunarYears .. convert(tonumber(string.sub(_nis, i, i)))
    end
    local LunarYear = string.gsub(_LunarYears, "零", "〇")
    local date1 = cTianGan[(LYear - 4) % 10 + 1]
        .. cDiZhi[(LYear - 4) % 12 + 1]
        .. "年（"
        .. cShuXiang[(LYear - 4) % 12 + 1]
        .. "）"
        .. LunarMonth
        .. cDayName[LDay]
    local date2 = LunarYear .. "年" .. LunarMonth .. cDayName[LDay]
    return date1, date2
end

local M = {}
local keys = {}

function M.setup(config, prefix)
    keys.today = config:get_string(prefix .. "/today") or "lunar"
    keys.convert = config:get_string(prefix .. "/convert") or "N"
end

function M.match(input)
    if input == keys.today then return true end
    if string.sub(input, 1, 1) == keys.convert and string.find(string.sub(input, 2), "^%d%d%d%d%d%d%d%d$") then return true end
    return false
end

function M.translate(input, seg)
    local r = {}
    if input == keys.today then
        local d1, d2 = Date2LunarDate(os.date("%Y%m%d"))
        table.insert(r, { d2, 999 })
        table.insert(r, { d1, 999 })
    elseif string.sub(input, 1, 1) == keys.convert then
        local d1, d2 = Date2LunarDate(string.sub(input, 2))
        table.insert(r, { d2, 999 })
        table.insert(r, { d1, 999 })
    end
    return r
end

return M
